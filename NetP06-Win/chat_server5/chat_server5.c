...
int main(int argc, char* argv[]) {
	char buf[BUF_LEN], buf1[BUF_LEN], buf2[BUF_LEN], buf5[BUF_LEN];
	char buf3[BUF_LEN], buf4[BUF_LEN];
	int i, j, n, ret, k, m, p;
		...
		for (i = 0; i < num_chat; i++) {
			...
			if (FD_ISSET(client_fds[i], &read_fds)) {
				...
				if (strncmp(buf1, CHAT_CMD_LOGIN, strlen(CHAT_CMD_LOGIN)) == 0) { // "/login"
					...
					for (j = 0; j < num_chat; j++) {
						if (j != i) { // 본인 제외 다른 사용자에게 입장을 알린다.
							sprintf(buf2, "[%s] %s", userlist[i], "님이 입장하였습니다.\n");
				...
				if (strncmp(buf + n + 1, CHAT_CMD_TO, strlen(CHAT_CMD_TO)) == 0) { // "/to"
					...
					m = strlen(buf2) + 1;
					p = strlen(buf3) + 1;
					strcpy(buf4, buf + k + m + p);
					// buf2 귓속말 받는 사람
					// buf1 -> 보내는 사람 || to -> /to 명령어 || messege ->  메세지
					for (j = 0; j < num_chat; j++) {
						if (strcmp(userlist[j], buf3) == 0) {
							sprintf(buf5, "[귓속말] %s %s", buf1, buf4);
				...
				// 1:1 채팅		
				if (strncmp(buf + n + 1, CHAT_CMD_WITH, strlen(CHAT_CMD_WITH)) == 0) {
					sscanf(buf, "%s %s %s", buf1, buf2, buf3);
					sprintf(buf4, "%s 님이 1:1 대화를 요청했습니다. (y/n)?\n", buf1);
					//buf1 = [유저의 이름], buf2 = /with, buf3 = with 하려는 유저의 이름
					for (j = 0; j < num_chat; j++) {
						if (strcmp(userlist[j], buf3) == 0) {
							userwith[i] = j;
							userwith[j] = i;
							if (send(client_fds[j], buf4, BUF_LEN, 0) < 0) { //1대1 요청..
								printf("client[%d] send error.", j);
								client_error[j] = 1;
								continue;
							}
							if (recv(client_fds[j], buf2, BUF_LEN, 0) < 0) {
								printf("client[%d] send error.", j);
								client_error[j] = 1;
								continue;
							}
							if (strncmp(buf + n + 1, CHAT_CMD_WITH_YES, strlen(CHAT_CMD_WITH_YES)) == 0) {
								printf("성사");
							}
...
void RemoveClient(int i) {
	...
	for (int j = 0; j < num_chat; j++) {
		...
		if (j != i) { // 본인 제외 다른 사용자에게 퇴장을 알린다.
			sprintf(buf2, "[%s] %s", userlist[i], "님이 퇴장하였습니다.\n");
			if (send(client_fds[j], buf2, BUF_LEN, 0) < 0) {
				printf("client[%d] send error.", j);
				continue;
			}
