...
int main(int argc, char* argv[]) {
	char buf[BUF_LEN], buf1[BUF_LEN], buf2[BUF_LEN], whis[BUF_LEN];
	char to[BUF_LEN], messege[BUF_LEN];
	int i, j, n, ret , k ,m, p ;
		...
		for (i = 0; i < num_chat; i++) {
			...
			if (FD_ISSET(client_fds[i], &read_fds)) {
				...
				if (strncmp(buf + n + 1, CHAT_CMD_TO, strlen(CHAT_CMD_TO)) == 0) { // "/to"
					sscanf(buf, "%s %s %s", buf1, to, buf2);
					k = strlen(buf1) +1 ;
					m = strlen(to) + 1;
					p = strlen(buf2) +1 ;
					strcpy(messege, buf + k + m + p);					
					// buf2 귓속말 받는 사람
					// buf1 -> 보내는 사람 || to -> /to 명령어 || messege ->  메세지
					for (j = 0; j < num_chat; j++) {
					if (strcmp(userlist[j], buf2) == 0) {
							sprintf(whis, "[귓속말] %s %s", buf1 , messege);
							if (send(client_fds[j], whis, BUF_LEN, 0) < 0) {
								printf("client[%d] send error.", j);
								client_error[j] = 1;
								continue;
							}
							memset(whis, 0, BUF_LEN);
							memset(buf, 0, BUF_LEN);
						}
					}
