...
public class JavaChatServer extends JFrame {
		...
		public void run() {
			while (true) { // 사용자 접속을 계속해서 받기 위해 while문
				try {
					...
					client_socket = socket.accept(); // accept가 일어나기 전까지는 한무 대기중
	...
	class UserService extends Thread {
		...
		public void list() {
			...
			for (int i = 0; i < user_vc.size(); i++) {
				UserService user = (UserService) user_vc.elementAt(i);				
				WriteOne(i + "\t" + user.UserName + "\t" + user.UserStatus);
			}

		}
		public void to(String name, String text) {
			for(int i = 0;i < user_vc.size(); i++) {
				UserService user = (UserService) user_vc.elementAt(i);
				if(name.equals(user.UserName)) {
					user.WriteOne("[귓속말]" +"[" + UserName +"]"+ text);
				}
				
			}
			
		}
		public boolean isSleep(String status) {
			if(status.equals("S"))
				return true;
			return false;			
		}
		...
		public void run() {
			while (true) { // 사용자 접속을 계속해서 받기 위해 while문
				try {
					...
					else if(msg[1].equals("/sleep")) {
						UserStatus = "S";
					}
					else if(msg[1].equals("/wakeup")) {
						UserStatus = "O";
					}		
