// game_client1.c 
// game server 에 접속한 nickname을 화면에 방향키로 움직이면서 표시한다.
// 다른 user들의 움직임도 같이 표시한다.
//
//
#include <stdio.h>
#include <conio.h>
...
void sendmypos(int sd, char *name, int oldx, int oldy, int newx, int newy)
{
	...
	send(sd, buf, BUF_LEN, 0);  // 고정 길이 전송.
}
...
void main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if (kbhit()==1) { // 내가 방향키 누르면.
			...
			//Sleep(Delay);
			//movename(myname, newx, newy, newx, newy);
		}
	}
}